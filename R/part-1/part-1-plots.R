...
setwd("~/Documents/courses/spring-2017/big-data/bigDataProject/R/part-1/")
...
all.data <- sparklyr::spark_read_csv(sc = sc, name = "data", path = "~/Downloads/311-reduced/311-reduced.csv",
...
ggsave(not.aggregated.plot, height = 8, width = 7, 
       ...
       filename = "bar-complaints-not-aggregated.png")
...
ggsave(aggregated.plot, height = 8, width = 7, 
       units = "in", device = "png", path = ".",
       filename = "bar-complaints-aggregated.png")
...
complaints.across.boroughs.plot.no.none <-
  complaints.across.boroughs %>%
  arrange(count %>% desc) %>%
  mutate_at(
    .cols = vars(Borough, Complaint_Type),
...
ggsave(complaints.across.boroughs.plot.no.none, height = 9, width = 8,
       units = "in", device = "png", path = ".",
       filename = "aggregated-complaints-by-borough-no-none.png")
